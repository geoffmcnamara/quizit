...
#     Note: you can escape a ";" with "\;" <-- it will not be considered a delimiter but will still get printed as ";"
...
def celebrate():
    ...
    if args['-c']:
        if file_exists(celebrate_file):
                ...
                if bool(re.search('[a-zA-Z]', line)):  # blank lines will get skipped
                    run_cmd(line)
    print("\n")
    msg = "You did great!"
    sayit(msg, prnt=False)
...
def chk_syntax(lines):
    ...
    for line in lines:
        ...
        line = line.partition('#')[0]  # this ignores trailing comments - turns the line blank if it is all a comment line
...
def main():
    ...
    for line in lines:
        ...
        print("\n"*1)
        printit("Score: " + str(score) + " out of " + str(scored_ques_cnt) + " scored questions.", center=True)
        printit("Your percent correct = " + str(round(score*100/scored_ques_cnt,2)) +"%", center=True)
    printit("=" * 40, center=True)    
    end = time.time()
    elapsed_time = (end - start) - remove_time
    msg = "The elapsed time: {:0.2f} seconds (minus celebration time)".format(elapsed_time)
    ...
    if USER != "none":  # This should always be the case if FORCE_USER is true
        ...
        msg = "User: {:15}, Dtime: {}, DatFile: {:25}, Score: {:>6}%, Elapsed_time: {:0.2f}, Num ques: {:6}".format(USER,dtime,BASENAME_DAT_FILE,pcnt,end-start,ques_cnt)
        global OUT_FILE
...
def handleOPTS(args):
    ...
    if args['-R']:
        ...
        # this will open an xterm as big as the current one with top left screen location 30+30
        term_cmds = {
